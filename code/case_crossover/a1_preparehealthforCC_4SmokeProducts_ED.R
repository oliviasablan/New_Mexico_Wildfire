# --------------------------------------------------------------------------------------------
# Title: a1_preparehealthforCC_4SmokeProducts.R
...
ED_sub <- ED %>%
  ...
  select("Date","Zip","allresp1","asthma1","COPD1","pneumonia1","bronchitis1","allcardio1",
...
rm(allresp_full, asthma_full,copd_full,pneumonia_full,bronchitis_full,
   cardio_full,cardiacarrest_full, arrhythmia_full,heartfailure_full,
   ischemic_full,myocardial_full,cerebrovascular1_full, ED_full, ED_sub, ref_dates)

# Need to merge the case crossover for the ED data with the lagged smoke exposure
# Will loop through this process for the 4 smoke exposures 

# Define the input file names from the previous code:
infiles = c('smoke_expKATE.rds', 'smoke_expBONNE.rds', 'smoke_expBONNE_KOvgp.rds', 'smoke_expKAMAL.rds', 'smoke_expKAMALmax.rds')

#Define the output file names
outfiles = c('casecross_list2022_KATE_ED_ALLDIAG.rds', 'casecross_list2022_BONNE_ED.rds', 'casecross_list2022_BONNE_KOvgp_ED.rds',
             'casecross_list2022_KAMAL_ED.rds', 'casecross_list2022_KAMALMAX_ED.rds')

# Read in population-weighted heat index data from FSU to be merged with ED and smoke data
...
for (i in (1:5)){
  ...
  # Read in the smoke exposure from "preparesmokeforCCloop.R"
  ...
  casecross_list_withsmoke <- casecross_list %>%
    map(~left_join(.,smoke_exp %>%
                   ...
                   by=c("Date","Zip")))
  # Merge with pop-weighted heat index data
